...
const BettingPage = () => {
  ...
  const getPicksForTable = (event) => {
    const result = allPicks.map(x => {
      ...
      return (<StyledTableCell 
        key={`${event.id}_${x}`} 
        style={{backgroundColor: betslip.some(y => y.pick.id === pick.id) ? 'green' : 'transparent'}}
  ...
  const calculateMaxWin = (payin, bslip) => {
    const maxWin = Math.round(
      bslip.reduce((acc, element) => 
        acc * element.pick.odds, payin) * 100) / 100;
    setWinAmount(maxWin);
  }
  ...
  const payinTicket = async () => {
    ...
    const payinRequest = {
      ...
      ticketBets: betslip.map(x => ({pickId: x.pick.id, odds: x.pick.odds}))
    }

    const result = await apiContext.fetchData('post', '/Ticket/Payin', payinRequest)
  ...
  return offerState ? (
    <div className="betting-page-main-container">
      <Table size="small" sx={{ marginTop: "50px" }} className='betting-page-offer'>
        <TableHead>
          <TableRow>
      ...
      <div className="betting-page-betslip">
          ...
          {betslip.map(x => (
            <div className="betting-page-betslip-element" key={x.pick.id}>
              ...
              <p><b>{x.pick.pickName}</b>{"\u00A0"}{x.pick.odds}</p>
            </div>
          ))}
          <div className='betting-page-betslip-inputs-container'>
            <input type='number' value={payinAmount} onChange={onPayinValueChanged} />
